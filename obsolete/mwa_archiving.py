...
import socket
import time
from archive_processing.utils.mwa_metadata import MWAFileTypeFlags
...
class NGASHttpPushConnector(object):
    ...
    def transfer_file(self, fullpath):
        ...
        try:
            ...
            if sent != filesize:
                raise Exception(
                    f"data sent does not match filesize: {str(sent)} !="
                    f" {str(filesize)}"
                )
...
def archive_file(
    ...
    sql = (
        "INSERT INTO data_files (observation_num, filetype, size, filename,"
        " site_path, remote_archived) VALUES (%s, %s, %s, %s, %s, true)"
    )
    sql_params = [
        str(obs_id),
        str(file_type.value),
    ...
    try:
        ...
        if rows_affected == 1:
            try:
                ...
                ngas_conn = NGASHttpPushConnector(
                    ngas_url,
                    "QARCHIVE",
                    ngas_user,
                    ngas_pass,
                ...
                # http code of 200 means we're good. If not raise exception
...
def pawsey_stage_files(file_list: list, host: str, port: int):
    ...
    try:
        ...
        json_output = json.dumps(
            {"files": [os.path.split(f)[1] for f in file_list]}
        )
