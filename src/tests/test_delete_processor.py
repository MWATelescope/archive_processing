...
def aws_credentials():
    ...
    ] = "https://ingest.pawsey.org.au,https://vss-1.pawsey.org.au:9000"
...
def test_generate_data_structures(delete_processor):
    ...
    assert delete_requests == {1: [11, 12, 13], 2: [21, 22, 23], 3: [31, 32, 33]}
...
def get_obs_data_files_filenames_except_ppds(delete_processor):
    assert delete_processor.repository.get_not_deleted_obs_data_files_except_ppds(
        11
    ) == [
        {
            "location": 2,
...
def test_delete_files(delete_processor):
    ...
    with pytest.raises(Exception):
        delete_processor.repository.update_files_to_deleted(
            failing_mock_delete_function, None, obs_files
        )
    ...
    delete_processor.repository.update_files_to_deleted(
        passing_mock_delete_function, None, [file["filename"] for file in obs_files]
    )
...
def test_run(delete_processor, aws_credentials):
    ...
    unactioned_delete_requests = delete_processor.repository.run_sql_get_many_rows(
        sql, None
    )
